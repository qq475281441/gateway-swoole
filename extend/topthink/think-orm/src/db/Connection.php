...
abstract class Connection
{
	...
	public function getFieldBindType($type)
	{
		if (0 === strpos($type, 'set') || 0 === strpos($type, 'enum')) {
			$bind = PDO::PARAM_STR;
		} else if (preg_match('/(double|float|decimal|real|numeric)/is', $type)) {
			$bind = self::PARAM_FLOAT;
		} else if (preg_match('/(int|serial|bit)/is', $type)) {
			$bind = PDO::PARAM_INT;
	...
	{
		//		return MysqlConnectPool::getInstance()->getConnect();
	...
	public function update(Query $query)
	{
		...
		} else if (!isset($key) && is_string($pk) && isset($options['where']['AND'])) {
	...
	public function delete(Query $query)
	{
		...
		if (isset($options['cache']) && is_string($options['cache']['key'])) {
			$key = $options['cache']['key'];
		} else if (!is_null($data) && true !== $data && !is_array($data)) {
			$key = $this->getCacheKey($query, $data);
		} else if (is_string($pk) && isset($options['where']['AND'])) {
			foreach ($options['where']['AND'] as $val) {
				if (is_array($val) && $val[0] == $pk) {
					$key = $this->getCacheKey($query, $val);
				}
		...
		foreach ($bind as $key => $val) {
			...
			if (is_array($val)) {
				if (PDO::PARAM_INT == $val[1] && '' === $val[0]) {
					$val[0] = 0;
				} else if (self::PARAM_FLOAT == $val[1]) {
					$val[0] = (float)$val[0];
					$val[1] = PDO::PARAM_STR;
				}
	...
	{
		var_dump('自带的close》》》》》》》》》》》》》》》》》》》》》》》》》');
		//		MysqlConnectPool::getInstance()->release($this->linkID);
		$this->linkID    = null;
		$this->linkWrite = null;
		$this->linkRead  = null;
		$this->links     = [];
	...
	protected function getCacheKey(Query $query, $value)
	{
		if (is_scalar($value)) {
			$data = $value;
		} else if (is_array($value) && isset($value[1], $value[2]) && in_array($value[1], ['=', 'eq'])) {
			$data = $value[2];
		}
	...
	private static function parseConfig($config)
	{
		if (empty($config)) {
			$config = Db::getConfig();
		} else if (is_string($config) && false === strpos($config, '/')) {
			// 支持读取配置参数
